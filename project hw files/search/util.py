...
class PriorityQueue:
  ...
  def popWithPriority(self):
    """
      Returns lowest-priority item and its priority in priority queue, or
      None if the queue is empty
    """
    if self.isEmpty(): return None
    (priority,item) = heapq.heappop(self.heap)
  ...
  def check_pop(self):
    """
      Returns lowest-priority item in priority queue
      without deleting it from priority queue, or
      None if the queue is empty
    """
    if self.isEmpty(): return None
    (priority,item) = heapq.heappop(self.heap)
